<div>
    <form name="frmEvent" action="{{route('manage.events.update', $event->id)}}" method="post">
        @csrf
        @method('PUT')
        ...
        <div class="form-group {{ $errors->has('venue') ? 'has-error' : '' }}">
            ...
            <input type="text" id="venue" name="venue" class="form-control" value="{{ old('venue', isset($event) ? $event->venue: '') }}" required>
            @if($errors->has('venue'))
                <div class="invalid-feedback">
                    {{ $errors->first('venue') }}
                </div>
        ...
        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
            ...
            <input type="description" id="description" name="description" class="form-control" value="{{ old('description', isset($event) ? $event->description: '') }}">
        ...
        <div class="form-group {{ $errors->has('booking_link') ? 'has-error' : '' }}">
            ...
            <input type="text" id="booking_link" name="booking_link" class="form-control" value="{{ old('booking_link', isset($event) ? $event->booking_link: '') }}">
        ...
        @if($event->image)
                   <img id="holder" style="margin-top:15px;max-height:100px;" src="{{ $event->image }}" />
        @endif
